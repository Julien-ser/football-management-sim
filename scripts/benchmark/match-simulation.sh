...
# Performance Benchmarks for Football Manager Simulator
# Run these scripts to measure simulation performance and detect regressions

set -e

echo "=========================================="
echo "Football Manager Simulator - Benchmarks"
echo "=========================================="
echo ""
...
echo "Running match simulation benchmark..."
echo "This will simulate 10 matches and measure average time per match minute"
echo ""
...
const warmup = new MatchSimulator(homeTeam, awayTeam, tactics, tactics);
warmup.simulate();
...
for (let i = 0; i < numRuns; i++) {
    ...
    console.log('  Run \${i+1}: \${durationMs.toFixed(2)}ms (\${(durationMs/90).toFixed(2)}ms per minute)');
}
